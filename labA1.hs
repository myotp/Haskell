{-|
  ...
  ./labA1 1 +RTS -N2 -l -RTS     ## Linear scanl1
...
-}
par_scanl1 f xs = par_scanl1' 4 f xs
...
scan0 = sum (scanl1 slow_plus [10000..10500])

-- parallel implementation with rpar
scan1 = sum (par_scanl1 slow_plus [10000..10500])

-- mainly copied rpar.hs from PCPH
-- <<main
main = do
  [n] <- getArgs
  let scan_fun = [scan0, scan1] !! (read n - 1)
  t0 <- getCurrentTime
  printTimeSince t0
  print scan_fun
  printTimeSince t0
